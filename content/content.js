...
function showKeyModal(onSubmit) {
    ...
    overlay.querySelector("#hc-cancel").onclick = (e) => { e.preventDefault(); overlay.remove(); };
...
async function addGame(id) {
    const r = await fetch("http://localhost:3000/" + id, { method: "POST" });
    if (r.ok) {
    ...
    if (r.status === 401) {
        showKeyModal(async (key) => {
            const keyRes = await fetch("http://localhost:3000/key", { method: "POST", body: key });
            if (keyRes.ok) {
                const retry = await fetch("http://localhost:3000/" + id, { method: "POST" });
                retry.ok ? showToast("Game added!") : showToast("Error: " + await retry.text(), true);
            } else {
                showToast("Key inválida: " + await keyRes.text(), true);
                showKeyModal(arguments.callee);
            }
        });
        return;
    }
